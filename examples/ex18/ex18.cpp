...
/*Delare Functions*/
void ApplyForce(double Time, double fext, double tMax);
...
int main(int argc, char **argv) {
  ...
  if (ImplicitStatic) {
    // Static solution
    double fext = 0.001; //max force in Newtons
    double dMax = 0.0; //max diplacement in meters
    double tMax = 1.0;
    ShapeFunctions();
    ReadMaterialProperties();
    ApplyForce(Time, fext, tMax);
...
void ApplyForce(double Time, double fext, double tMax){
  ...
  printf("Time = %10.5e, Applied Force = %10.5e\n", Time, AppliedForce);
  return;
}
void ApplyBoundaryConditions(double Time, double dMax, double tMax) {
  double tol = 1e-5;
