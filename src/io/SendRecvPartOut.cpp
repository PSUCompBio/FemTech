...
PARTOUTPUT *SendReceivePartitioningOutput(const idx_t edgecut, idx_t *part, const int partArraySize)
{
    ...
    if (world_rank == 0)
    {
        ...
        for (int i = 1; i < world_size; i++)
        {
            ...
            MPI_Recv(&Result[i].part_size, 1, MPI_INT, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
