...
//-------------------------------------------------------------------------------------------
static bool IsElementSection(char *Line, FILE *File,
                             int *NDimExceptRuleExists = NULL) {
...
bool ReadLsDyna(const char *FileName) {
  ...
  while (fgets(Line, sizeof(Line), File) != NULL) {
    if (IsElementSection(Line, File, &NDimExceptRuleExists)) {
      ...
      while (fgets(Line, sizeof(Line), File) != NULL) {
        if (LineToArray(true, false, 2, 1, Line) == 0 ||
            LineToArray(true, true, 3, 0, Line) == 0) {
          break;
        }
        nallelements++;
      ...
      if (fseek(File, LastValidElemDataLinePos, SEEK_SET) != 0) {
        FILE_LOG_SINGLE(ERROR,
                        "'fseek()' call for LastValidElemDataLinePos failed");
      }
      ...
      while (!Dim3Exists && fgets(Line, sizeof(Line), File) != NULL) {
        if (ndim == 0 && strstr(Line, "nid") != NULL) {
          ...
          double *ZColumnValue;
          if (LineToArray(false, false, 4, 1, Line, Delim,
                          (void **)&ZColumnValue) > 0) {
      ...
      // Even each row in Z column of NODE section is 0, ndim will be 3 IF input
      // file contains ELEMENT_BEAM section For ELEMENT_SHELL and ELEMENT_SOLID
      // files, ndim will be 2 IF there are not non-zero rows in Z column
      if (ndim == 2 || ndim == 3) {
  ...
  int fseeked = 0;
  if (nallelements == 0 || NodesSectionPos == -1 ||
      (fseeked = fseek(File, ElementsSectionPos, SEEK_SET)) != 0) {
    fclose(File);
    if (nallelements == 0) {
      FILE_LOG_SINGLE(ERROR, "No element found. This means input file is empty "
                             "or contains invalid data");
    }
    if (NodesSectionPos == -1) {
      FILE_LOG_SINGLE(ERROR, "Node section not found. This means input file is "
  ...
  // Fixing range of lines (in elements section of mesh file) from which
  // processor will read its own elements
  const int From = world_rank * (nallelements / world_size);
  const int To = From + nelements - 1;

  // Creating and initializing "eptr" array, and determining size of
  // "connectivity" array of processor
  ...
  if (ConnectivitySize != eptr[nelements] ||
      fseek(File, ElementsSectionPos, SEEK_SET) != 0) {
    fclose(File);
    FreeArrays();
    if (ConnectivitySize != eptr[nelements]) {
  ...
  // Creating and initializing "connectivity", "pid" and "ElementType" arrays
  // for processor
  ...
  while (fgets(Line, sizeof(Line), File) != NULL) {
    if (IsElementSection(Line, File)) {
      ...
      while (fgets(Line, sizeof(Line), File) != NULL) {
        ...
        const int ne =
            LineToArray(true, false, 1, 1, Line, Delim, (void **)&eids);
        const int np =
            LineToArray(true, false, 2, 1, Line, Delim, (void **)&PIDs);
        const int nn =
            LineToArray(true, true, 3, 0, Line, Delim, (void **)&Nodes);
        ...
        if (i >= From && i <= To) {
          ...
          if (N > 4) {
            if (!reducedIntegration)
              ...
              strcpy(ElementType[pi], "C3D8R");
          } else if (N == 4) {
            strcpy(ElementType[pi], "C3D4");
          } else if (N == 2) {
            strcpy(ElementType[pi], "T3D2");
          } else {
            FILE_LOG_SINGLE(ERROR, "Unknown element type \"%d\" in line %s", N,
                            Line);
          }
  ...
  const int UniqueConnectivitySize =
      unique(UniqueConnectivity, ConnectivitySize);
  const int csize = ndim * sizeof(double);
  double *UniqueConnCoordinates =
      (double *)calloc(UniqueConnectivitySize, csize);
  const int nCols = ndim + 1;
  int nCopied = 0;
  while (nCopied < UniqueConnectivitySize &&
         fgets(Line, sizeof(Line), File) != NULL) {
    double *NodeData;
    const int n =
        LineToArray(false, false, 1, nCols, Line, Delim, (void **)&NodeData);
    if (n == nCols) {
      const int NodeID = int(NodeData[0]) - 1;
      const int *p =
          (const int *)bsearch(&NodeID, UniqueConnectivity,
                               UniqueConnectivitySize, sizeof(int), compare);
      if (p != NULL) {
  ...
  for (int i1 = 0; i1 < ConnectivitySize; i1++) {
    const int *p =
        (const int *)bsearch(&connectivity[i1], UniqueConnectivity,
                             UniqueConnectivitySize, sizeof(int), compare);
    if (p != NULL) {
      const int I = p - UniqueConnectivity;
  ...
  if (nNodes != ConnectivitySize) {
    ...
    FILE_LOG_SINGLE(ERROR, "nnodes = %d, ConnectivitySize = %d, ndim = %d",
